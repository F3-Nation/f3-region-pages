...
SECRET_VARS=("POSTGRES_URL" "F3_DATA_WAREHOUSE_URL" "SEED_API_KEY")
SECRET_IDS=("postgres-url" "f3-data-warehouse-url" "seed-api-key")
...
validate_env_file() {
  ...
  if [[ ! -f "$env_file" ]]; then
    ...
    log_error "Required variables: POSTGRES_URL, F3_DATA_WAREHOUSE_URL, SEED_API_KEY"
    return 1
  fi
