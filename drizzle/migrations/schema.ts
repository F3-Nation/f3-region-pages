import { pgTable, foreignKey, serial, varchar, integer, json, timestamp, index, boolean, date, doublePrecision, text, uuid, real, pgEnum } from "drizzle-orm/pg-core"
import { sql } from "drizzle-orm"

export const achievementCadence = pgEnum("achievement_cadence", ['weekly', 'monthly', 'quarterly', 'yearly', 'lifetime'])
export const achievementThresholdType = pgEnum("achievement_threshold_type", ['posts', 'unique_aos'])
...
export const eventCategory = pgEnum("event_category", ['first_f', 'second_f', 'third_f'])
export const orgType = pgEnum("org_type", ['ao', 'region', 'area', 'sector', 'nation'])
export const regionRole = pgEnum("region_role", ['user', 'editor', 'admin'])
export const requestType = pgEnum("request_type", ['create_location', 'create_event', 'edit', 'delete_event'])
export const updateRequestStatus = pgEnum("update_request_status", ['pending', 'approved', 'rejected'])
...
export const users = pgTable("users", {
	...
	meta: json(),
	created: timestamp({ mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
	updated: timestamp({ mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
...
export const eventInstances = pgTable("event_instances", {
	...
	index("idx_event_instances_is_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_event_instances_location_id").using("btree", table.locationId.asc().nullsLast().op("int4_ops")),
	index("idx_event_instances_org_id").using("btree", table.orgId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const locations = pgTable("locations", {
	...
	index("idx_locations_is_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_locations_name").using("btree", table.name.asc().nullsLast().op("text_ops")),
	index("idx_locations_org_id").using("btree", table.orgId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.orgId],
			foreignColumns: [orgs.id],
...
export const attendanceXAttendanceTypes = pgTable("attendance_x_attendance_types", {
...
export const events = pgTable("events", {
	...
	index("idx_events_is_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_events_location_id").using("btree", table.locationId.asc().nullsLast().op("int4_ops")),
	index("idx_events_org_id").using("btree", table.orgId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.locationId],
			foreignColumns: [locations.id],
...
export const eventInstancesXEventTypes = pgTable("event_instances_x_event_types", {
	eventInstanceId: integer("event_instance_id").notNull(),
...
export const eventTagsXEventInstances = pgTable("event_tags_x_event_instances", {
	eventInstanceId: integer("event_instance_id").notNull(),
...
export const eventTagsXEvents = pgTable("event_tags_x_events", {
...
export const eventsXEventTypes = pgTable("events_x_event_types", {
	eventId: integer("event_id").notNull(),
	eventTypeId: integer("event_type_id").notNull(),
}, (table) => [
	index("idx_events_x_event_types_event_id").using("btree", table.eventId.asc().nullsLast().op("int4_ops")),
	index("idx_events_x_event_types_event_type_id").using("btree", table.eventTypeId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.eventId],
			foreignColumns: [events.id],
...
export const expansionsXUsers = pgTable("expansions_x_users", {
	expansionId: integer("expansion_id").notNull(),
	userId: integer("user_id").notNull(),
	requestDate: timestamp("request_date", { mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
	notes: varchar(),
}, (table) => [
	foreignKey({
...
export const orgsXSlackSpaces = pgTable("orgs_x_slack_spaces", {
	orgId: integer("org_id").notNull(),
...
export const positionsXOrgsXUsers = pgTable("positions_x_orgs_x_users", {
...
export const rolesXPermissions = pgTable("roles_x_permissions", {
...
export const rolesXUsersXOrg = pgTable("roles_x_users_x_org", {
	roleId: integer("role_id").notNull(),
	userId: integer("user_id").notNull(),
	orgId: integer("org_id").notNull(),
}, (table) => [
...
export const achievementsXUsers = pgTable("achievements_x_users", {
	achievementId: integer("achievement_id").notNull(),
	userId: integer("user_id").notNull(),
	dateAwarded: timestamp("date_awarded", { mode: 'string' }).default(sql`timezone('utc'::text, now())`).notNull(),
	awardYear: integer("award_year").default(sql`'-1'`).notNull(),
	awardPeriod: integer("award_period").default(sql`'-1'`).notNull(),
}, (table) => [
	foreignKey({
...
export const orgs = pgTable("orgs", {
	...
	index("idx_orgs_is_active").using("btree", table.isActive.asc().nullsLast().op("bool_ops")),
	index("idx_orgs_org_type").using("btree", table.orgType.asc().nullsLast().op("enum_ops")),
	index("idx_orgs_parent_id").using("btree", table.parentId.asc().nullsLast().op("int4_ops")),
	foreignKey({
